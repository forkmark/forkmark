...
class TestStoreDuckDB:
    """Run core store operations on the DuckDB backend to verify parity."""

    @pytest.fixture(autouse=True)
    def setup_db(self, tmp_path):
        from core.store import Database
        db_path = str(tmp_path / "test_duck.db")
        self.db = Database(db_path, trace_backend="duckdb")

    def test_workflow_crud(self):
        wf = self.db.upsert_workflow("duck-wf", "duckdb test")
        assert wf.name == "duck-wf"
        fetched = self.db.get_workflow(wf.id)
        assert fetched.name == "duck-wf"
        wfs = self.db.list_workflows()
        assert len(wfs) >= 1

    def test_upsert_workflow_idempotent(self):
        wf1 = self.db.upsert_workflow("idempotent-duck")
        wf2 = self.db.upsert_workflow("idempotent-duck", "updated")
        assert wf1.id == wf2.id
        assert wf2.description == "updated"

    def test_run_lifecycle(self):
        wf = self.db.upsert_workflow("run-duck")
        run = self.db.create_run(wf.id, {"q": "hello"})
        assert run.status.value == "running"
        self.db.complete_run(run.id)
        run2 = self.db.get_run(run.id)
        assert run2.status.value == "completed"

    def test_branch_and_step(self):
        wf = self.db.upsert_workflow("branch-duck")
        run = self.db.create_run(wf.id)
        br = self.db.create_branch(run.id, wf.id, "b1", "gpt-4o")
        step = self.db.save_step_output(
            run_id=run.id, branch_id=br.id, step_name="classify",
            step_index=0, input_messages=[{"role": "user", "content": "hi"}],
            output_text="hello world", model_id="gpt-4o",
        )
        assert step.output_text == "hello world"
    ...
    def test_comparison_and_decision(self):
        wf = self.db.upsert_workflow("comp-duck")
        run = self.db.create_run(wf.id)
        b1 = self.db.create_branch(run.id, wf.id, "a", "gpt-4o")
        b2 = self.db.create_branch(run.id, wf.id, "b", "claude-3-5-sonnet")
        self.db.save_step_output(run.id, b1.id, "step1", 0, [], "output A", "gpt-4o")
        self.db.save_step_output(run.id, b2.id, "step1", 0, [], "output B", "claude-3-5-sonnet")
        comp = self.db.create_comparison(run.id, wf.id, b1.id, b2.id, step_names=["step1"])
        assert comp.scoring_status in ("completed", "pending")

    def test_api_key_create_verify(self):
        meta, raw = self.db.create_api_key("duck-key")
        assert raw.startswith("fm_")
        verified = self.db.verify_api_key(raw)
        assert verified is not None
        assert verified.name == "duck-key"

    def test_settings(self):
        self.db.set_setting("duck_key", "duck_val")
        val = self.db.get_setting("duck_key")
        assert val == "duck_val"

    def test_stats(self):
        self.db.upsert_workflow("stats-duck")
        stats = self.db.get_stats()
        assert "total_workflows" in stats
        assert stats["total_workflows"] >= 1
    ...
    def test_eval_run_lifecycle(self):
        from core.models import EvalRunStatus
        wf = self.db.upsert_workflow("eval-duck")
        er = self.db.create_eval_run(
            wf.id, "eval-duck-1",
            branch_a_config={"model": "gpt-4o"},
            branch_b_config={"model": "claude-3"},
        ...
        assert er.status == EvalRunStatus.PENDING
        self.db.update_eval_run_status(er.id, EvalRunStatus.RUNNING)
        self.db.update_eval_run_status(er.id, EvalRunStatus.COMPLETED, total_cases=5)
        fetched = self.db.get_eval_run(er.id)
        assert fetched.status == EvalRunStatus.COMPLETED
        assert fetched.total_cases == 5
...
class TestComparator:
    def test_lexical_identical(self):
        ...
        # IDF degeneracy: with only 2 identical docs all IDF weights → 0,
        # so TF-IDF cosine returns 0. SequenceMatcher gives 1.0 (30% weight).
        # divergence = 1 - (0*0.7 + 1.0*0.3) = 0.7
        assert _lexical_divergence("hello world", "hello world") == 0.7
    ...
    def test_tfidf_cosine(self):
        ...
        # Identical docs → all terms in both → df=2 → idf=log(1)=0 → cosine=0
        assert _tfidf_cosine("hello world", "hello world") == 0.0
        assert _tfidf_cosine("", "") == 1.0  # empty special-case
        sim = _tfidf_cosine("hello", "goodbye")
        assert 0.0 <= sim <= 1.0
